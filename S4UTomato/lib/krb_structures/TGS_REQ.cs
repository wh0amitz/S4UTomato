...
namespace S4UTomato
{
    ...
    {
        //                   TGS_REQ.NewTGSReq(userName, domain, userName, ticket, clientKey, etype, Interop.KERB_ETYPE.subkey_keymaterial, false, targetUser, false, false, opsec)
        // 构造 TGS_REQ 请求
        public static byte[] NewTGSReq(string userName, string domain, string sname, Ticket providedTicket, byte[] clientKey, Interop.KERB_ETYPE paEType, Interop.KERB_ETYPE requestEType = Interop.KERB_ETYPE.subkey_keymaterial, bool renew = false, string s4uUser = "", bool enterprise = false, bool roast = false, bool opsec = false, bool unconstrained = false, KRB_CRED tgs = null, string targetDomain = "", bool u2u = false, bool keyList = false)
        {
            ...
            {
                // set the username
                req.req_body.cname.name_string.Add(userName);    // 设置 cname
            }

            // get domain from service for cross domain requests
            // if not requesting a cross domain TGT (krbtgt)
            string[] parts = sname.Split('/');    // [svc, domain.com:1234]
            if (String.IsNullOrEmpty(targetDomain))
            {
                if (!(roast) && (parts.Length > 1) && (parts[0] != "krbtgt") && (tgs == null) && parts[0] != "kadmin")
                {
                    ...
                    if (String.IsNullOrEmpty(targetDomain))
                        targetDomain = domain;    // 这里 targetDomain 是 NULL，因此将 targetDomain 设置为 domain
                }
                else if (enterprise)
                {
            ...
            if (!String.IsNullOrEmpty(s4uUser))    // targetUser (/impersonateuser)
            {
                ...
                else
                {
                    ...
                    req.req_body.sname.name_string.Add(userName);    // 这里将 sname 设为 service 1的服务账户，因为要获取 service 1 的服务票据
                }
